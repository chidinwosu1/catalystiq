...
# Calendar-day warm-up fetched BEFORE the first prediction date so the
# 200-day SMA / regime and other long-lookback indicators have history.
...
def _ingest(symbols: list[str], benchmark: str, db, *, start: dt.date) -> list[str]:
    ...
    Note: ``ensure_fresh`` only re-fetches when Silver is *stale* (missing the
    latest session); it does not backfill older history into an already-current
    symbol. Run against a fresh DB to guarantee the requested range is pulled.
    """
